...
function menuDivFix(){
	//to fix the issue of the empty DIV being created in D8 templates
	//detach the result
...
// 		style: 'mapbox://styles/jamesdavy/cj977n9d117bl2rqe7z7clozd', //North Star
  ...
  map.addControl(new mapboxgl.FullscreenControl());
    ...
    //get the id of the layer above the background layer where we will insert the satellite imagery
    var layers = map.getStyle().layers;
    var layerId = layers[1].id;
...
/* 		$('div#block-mapmenuscope').on( 'click', '.view-content > .indicator-row.views-row', function() {
			...
			var nodeID = $(this).find(".views-field-nid").text().trim();
...
function removeIndicator(indicatorClass) {
...
/* 	//then we record this indicator as inactive (so the most recent query is saved) -- this is from the previous design to compare multiple indicators. 
	for (var key in selSettings.selIndicatorsActive) {
		selSettings.selIndicatorsActive[key] = 0;
	} */
...
function getRestResults(){
	...
	if (selSettings.selIndicatorCountries != ''){
		thisMap.setFilter("CountriesBadMask", buildFilter(selSettings.selIndicatorCountries, '!in', 'ISO3'));
		thisMap.setFilter("CountriesGoodMask", buildFilter(selSettings.selIndicatorCountries, 'in', 'ISO3'));
	...
	var indicator = selSettings.selIndicators;
...
function generateRestURL(){
	console.log(selSettings.selCoNUM);
	//we get the url from the hidden fields in the indicator menu
	var indicatorURL = selSettings.selIndicatorRESTurl
